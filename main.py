...
for i in range(times):
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = EGolden_SWOA(fit_func=Sphere, 
    ...
    x_max = 10*np.ones(d)
    x_min = -10*np.ones(d)
    optimizer = EGolden_SWOA(fit_func=Schwefel_P222, 
                             num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 100*np.ones(d)
    x_min = -100*np.ones(d)
    optimizer = EGolden_SWOA(fit_func=Quadric, 
                             num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    optimizer = EGolden_SWOA(fit_func=Rastrigin, 
                             num_dim=d, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
    ...
    x_max = 5*np.ones(2)
    x_min = -5*np.ones(2)
    optimizer = EGolden_SWOA(fit_func=Branin, 
                             num_dim=2, num_particle=p, max_iter=g, x_max=x_max, x_min=x_min)
    start = time.time()
    optimizer.opt()
    end = time.time()
